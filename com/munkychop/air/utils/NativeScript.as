package com.munkychop.air.utils
{
	...
	public class NativeScript extends EventDispatcher
	{
		...
		private function standardErrorDataHandler (event:ProgressEvent):void
		{
			...
			dispatchEvent (new NativeScriptEvent (NativeScriptEvent.NATIVE_SCRIPT_COMMAND_ERROR, {error: errorString}));
			
			trace ("NativeScript Error: " + errorString);
		}
		
		private function standardErrorIOErrorHandler (event:IOErrorEvent):void
		{
			dispatchEvent (new NativeScriptEvent (NativeScriptEvent.NATIVE_SCRIPT_COMMAND_ERROR, {error: event.toString()}));
			trace ("NativeScript IOError: " + event.toString());  
		}
	}
}